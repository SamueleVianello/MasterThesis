...
where $\mu$ and $\sigma$ are respectively the drift and the diffusion of the continuous part, $Y$ is a process modelling the intensity of the jumps and $N(t)$ is the Poisson process driving the arrival of the jumps and has parameter $\lambda$.
...
Following \cite{MERTON1976}, we take $y_k$ \textit{i.i.d.} (independent and identically distributed) and Gaussian, as we already stated.
...
This equation is a good analytical results but it is not so practical for calibration purposes, as we will explain more in depth in chapter \ref{chpr:calibration}.
